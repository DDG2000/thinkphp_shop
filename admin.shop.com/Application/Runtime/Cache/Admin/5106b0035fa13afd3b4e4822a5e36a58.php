...
.explode {
  background:url(http://admin.shop.com/Public/IMG/menu_minus.gif) no-repeat 0px 3px;
  font-weight:bold;
}
.collapse {
  background:url(http://admin.shop.com/Public/IMG/menu_plus.gif) no-repeat 0px 3px;
  font-weight:bold;
}
.menu-item {
  background:url(http://admin.shop.com/Public/IMG/menu_arrow.gif) no-repeat 0px 3px;
  font-weight:normal;
}
#help-title {
  font-size:14px;
    ...
    <p>
        <span style="float:right; padding:3px 5px;" >
            <a href="javascript:toggleCollapse();">
                <img id="toggleImg" src="http://admin.shop.com/Public/IMG/menu_minus.gif" width="9" height="9" border="0" alt="闭合" />
            </a>
        </span>
...
<div id="main-div">
    <div id="menu-list">
        <ul id="menu-ul">
            <!--遍历menus中的每条menu-->
            <?php if(is_array($menus)): foreach($menus as $key=>$top_menu): ?><!--顶级分类的层级等于1时遍历-->
                <?php if(($top_menu["level"]) == "1"): ?><li class="explode" key="" name="menu">
                    <?php echo ($top_menu["name"]); ?>
                        <ul>
                            <!--遍历menus中每条menu-->
                            <?php if(is_array($menus)): foreach($menus as $key=>$son_menu): ?><!--每条menu的父级id为当前top_menu的id时,在其内部遍历-->
                                <?php if(($son_menu["parent_id"]) == $top_menu['id']): ?><!--取出每条遍历菜单的url地址和名字-->
                                    <li class="menu-item"><a href="<?php echo U($son_menu['url']);?>" target="main-frame"><?php echo ($son_menu["name"]); ?></a></li><?php endif; endforeach; endif; ?>
                        </ul>
...
<script type="text/javascript" src="http://admin.shop.com/Public/JS/global.js"></script>
<script type="text/javascript" src="http://admin.shop.com/Public/JS/utils.js"></script>
<script type="text/javascript" src="http://admin.shop.com/Public/JS/transport.js"></script>
<script language="JavaScript">
    ...
    function toggleCollapse() {
        ...
        collapse = !collapse;
        document.getElementById('toggleImg').src = collapse ? 'http://admin.shop.com/Public/IMG/menu_minus.gif' : 'http://admin.shop.com/Public/IMG/menu_plus.gif';
